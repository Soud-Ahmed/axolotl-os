...
router.put('/:id', requireAuth, async (req, res) => {
    try {
        ...
        const { title, description, status, priority, assigneeId, dueDate } = req.body;
        const updates = {};
        if (title !== undefined && authReq.user.role !== 'client')
            updates.title = title;
        if (description !== undefined && authReq.user.role !== 'client')
            updates.description = description;
        if (status !== undefined)
            updates.status = status;
        if (priority !== undefined && authReq.user.role !== 'client')
            updates.priority = priority;
        if (assigneeId !== undefined && authReq.user.role !== 'client')
            updates.assigneeId = assigneeId;
        if (dueDate !== undefined && authReq.user.role !== 'client')

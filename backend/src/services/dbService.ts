...
class DbService {
  ...
  private localDbInitialized = false;
  ...
  private async initLocalDb() {
    if (this.localDbInitialized) return;
    try {
  ...
  private mapProfileToDb(user: Partial<User>): any {
    const dbRow: any = {};
    if (user.id !== undefined) dbRow.id = user.id;
    if (user.email !== undefined) dbRow.email = user.email;
    if (user.fullName !== undefined) dbRow.full_name = user.fullName;
    if (user.fullName === undefined && user.name !== undefined) dbRow.full_name = user.name;
    if (user.avatarUrl !== undefined) dbRow.avatar_url = user.avatarUrl;
    if (user.companyName !== undefined) dbRow.company_name = user.companyName;
    if (user.role !== undefined) dbRow.role = user.role;
    if (user.status !== undefined) dbRow.status = user.status;
    if (user.createdAt !== undefined) dbRow.created_at = user.createdAt;
    if (user.updatedAt !== undefined) dbRow.updated_at = user.updatedAt;
    return dbRow;
  }
  ...
  async getProfiles(): Promise<User[]> {
    if (this.supabase) {
      try {
        ...
        return db.profiles.map((p: any) => this.mapProfile(p));
      }
    } else {
      const db = await this.readLocalDb();
      return db.profiles.map((p: any) => this.mapProfile(p));
  ...
  async getProfileById(id: string): Promise<User | null> {
    if (this.supabase) {
      try {
        ...
        const db = await this.readLocalDb();
        const prof = db.profiles.find((p: User) => p.id === id);
  ...
  async getProfileByEmail(email: string): Promise<User | null> {
    if (this.supabase) {
      try {
        ...
        const db = await this.readLocalDb();
        const prof = db.profiles.find((p: any) => p.email.toLowerCase() === email.toLowerCase());
        if (!prof) return null;
  ...
  async getProfileWithPasswordByEmail(email: string): Promise<(User & { passwordHash: string }) | null> {
    if (this.supabase) {
      return null;
    } else {
      const db = await this.readLocalDb();
  ...
  async createProfileWithPassword(user: User, passwordHash: string): Promise<User> {
    if (this.supabase) {
  ...
  async updateProfile(id: string, updates: Partial<User>): Promise<User> {
    if (this.supabase) {
      try {
        ...
        const db = await this.readLocalDb();
        const idx = db.profiles.findIndex((p: User) => p.id === id);
  ...
  async getArticles(): Promise<Article[]> {
    if (this.supabase) {
      try {
        ...
        const db = await this.readLocalDb();
        return db.articles.map((art: any) => ({
          ...art,
          author: db.profiles.find((p: User) => p.id === art.authorId) || undefined,
        }));
      }
    } else {
      const db = await this.readLocalDb();
  ...
  async getArticleById(id: string): Promise<Article | null> {
    if (this.supabase) {
      ...
      const db = await this.readLocalDb();
      const art = db.articles.find((a: Article) => a.id === id);
      if (!art) return null;
      return {
        ...art,
        author: db.profiles.find((p: User) => p.id === art.authorId) || undefined,
  ...
  async createArticle(article: Omit<Article, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }): Promise<Article> {
  ...
  async updateArticle(id: string, updates: Partial<Article>): Promise<Article> {
    const now = new Date().toISOString();
    if (this.supabase) {
      const supabaseUpdates: any = { ...updates, updated_at: now };
      if (updates.publishDate !== undefined) supabaseUpdates.publish_date = updates.publishDate;
      if (updates.authorId !== undefined) supabaseUpdates.author_id = updates.authorId;
  ...
  async deleteArticle(id: string): Promise<boolean> {
    if (this.supabase) {
      ...
      const db = await this.readLocalDb();
      const filter = db.articles.filter((a: Article) => a.id !== id);
      if (filter.length === db.articles.length) return false;
      db.articles = filter;
      await this.writeLocalDb(db);
      return true;
    }
  ...
  async getTasks(): Promise<Task[]> {
    if (this.supabase) {
      try {
        ...
        const db = await this.readLocalDb();
        return db.tasks.map((task: any) => ({
          ...task,
          assignee: db.profiles.find((p: User) => p.id === task.assigneeId) || undefined,
          reporter: db.profiles.find((p: User) => p.id === task.reporterId) || undefined,
        }));
      }
    } else {
      const db = await this.readLocalDb();
  ...
  async createTask(task: Omit<Task, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }): Promise<Task> {
    const now = new Date().toISOString();
    const newTask: Task = {
  ...
  async updateTask(id: string, updates: Partial<Task>): Promise<Task> {
    ...
    if (this.supabase) {
      ...
      if (updates.reporterId !== undefined) supabaseUpdates.reporter_id = updates.reporterId;
      ...
      const db = await this.readLocalDb();
      const idx = db.tasks.findIndex((t: Task) => t.id === id);
  ...
  async deleteTask(id: string): Promise<boolean> {
    if (this.supabase) {
      ...
      const db = await this.readLocalDb();
      const filter = db.tasks.filter((t: Task) => t.id !== id);
      if (filter.length === db.tasks.length) return false;
      db.tasks = filter;
      await this.writeLocalDb(db);
      return true;
    }
  ...
  async getMediaAssets(): Promise<MediaAsset[]> {
    if (this.supabase) {
      try {
        ...
        const db = await this.readLocalDb();
        return db.media_assets.map((asset: any) => ({
          ...asset,
          uploader: db.profiles.find((p: User) => p.id === asset.uploadedBy) || undefined,
        }));
      }
    } else {
      const db = await this.readLocalDb();
  ...
  async createMediaAsset(asset: Omit<MediaAsset, 'id' | 'createdAt'> & { id?: string }): Promise<MediaAsset> {
    const now = new Date().toISOString();
  ...
  async deleteMediaAsset(id: string): Promise<boolean> {
    if (this.supabase) {
      ...
      const db = await this.readLocalDb();
      const filter = db.media_assets.filter((a: MediaAsset) => a.id !== id);
      if (filter.length === db.media_assets.length) return false;
  ...
  // --- Comments ---
  async getComments(articleId?: string, taskId?: string): Promise<Comment[]> {
    if (this.supabase) {
      ...
      if (articleId) comments = comments.filter((c: Comment) => c.articleId === articleId);
      if (taskId) comments = comments.filter((c: Comment) => c.taskId === taskId);
      return comments.map((c: any) => ({
        ...c,
        user: db.profiles.find((p: User) => p.id === c.userId) || undefined,
      }));
    }
  }
  ...
  async deleteComment(id: string): Promise<boolean> {
    if (this.supabase) {
      ...
      const db = await this.readLocalDb();
      const filter = db.comments.filter((c: Comment) => c.id !== id);
      if (filter.length === db.comments.length) return false;
      db.comments = filter;
      await this.writeLocalDb(db);
      return true;
    }
  ...
  async getActivityLogs(): Promise<ActivityLog[]> {
    if (this.supabase) {
      try {
        ...
        const db = await this.readLocalDb();
        return [...db.activity_logs].sort((a: ActivityLog, b: ActivityLog) => 
